-- ACTUALIZACIÓN DE EMISIÓN DE VOTOS
-- Añadiendo soporte para empresa_id en el RPC emitir_voto

DROP FUNCTION IF EXISTS votaciones.emitir_voto(uuid, uuid, uuid, numeric, uuid);
...
CREATE OR REPLACE FUNCTION votaciones.emitir_voto(
    ...
    p_empresa_id UUID DEFAULT NULL -- NUEVO PARAMETRO
)
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    -- 4. Insertar usando el usuario_id y empresa_id
    INSERT INTO votaciones.votos (usuario_id, pregunta_id, opcion_elegida_id, candidato_id, valor_numerico, empresa_id)
    VALUES (p_usuario_id, p_pregunta_id, p_opcion_id, p_candidato_id, p_valor_numerico, p_empresa_id);
END;
$$;

-- Permisos
GRANT EXECUTE ON FUNCTION votaciones.emitir_voto TO authenticated, anon;
